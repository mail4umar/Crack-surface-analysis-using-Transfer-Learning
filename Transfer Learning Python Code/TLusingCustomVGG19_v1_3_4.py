# -*- coding: utf-8 -*-
"""
Created on Tue Mar 15 11:45:27 2022

@author: Umar

HAVE PUT LOG FOR PROPERTY!!!!!! PLEASE CHECK
...
new in v1_3_4:
    select multipple inputs

new in v1_3_3:
    ability to select only one image/sample
    took out some options of image only or one property only
...
import numpy as np
save_the_file=False # FOR SAVING FEATRE + PROPERTY CSV FILE
scale_property=True # FOR SCALING VERY HIGH VALUES LIKE E
USE_ONLY_ADDITIONAL_INPUTS=False
Blind_test=True # TO ensure the data is truly hidden when splitting test and training
Unique_Images_Please=False # Only select one image per case
additional_input1_file=[]#["temp.csv"] #h2_full please IF you want h2_full. There is a condition
additional_input2_file=[]#"strain.csv" 
property_file='da_dn.csv'
desired_block='block5_pool'
PCA_components=9
...
path3="\\Python\\TransferLearning\\ford\\HT_Trans_PNG_o"
...
if Drop_Images_Please:
    ...
    for i in range(len(files)):
      ...
      if int(nameonly) in Drop_Images:
          #print(nameonly)
          #print(i)
...
if Unique_Images_Please: 
    ...
    for i in range(len(files)):
      ...
      nameonly = nameonly.split('_')[0]
      number=int(nameonly)
      image_sets[number].append(filename)
    
    files=[]
    for i in image_sets:
      sizer=len(image_sets[i])
      #print(sizer)
      random_number=random.randrange(sizer)
      files.append(image_sets[i][random_number])
#%% Read Images
...
for layer in model.layers:
    ...
    if layer.name in desired_block: #block3_conv4
        break
...
import pandas as pd
df=pd.read_csv(property_file)  # Change da_dn here to get another property
...
if not not additional_input1_file:
    df=pd.read_csv(additional_input1_file)  # Change da_dn here to get another property
    X_2=np.zeros((len(files),1))
    if additional_input1_file=="h2_full.csv" or additional_input1_file=="h2_Donut.csv":
        for i in range(len(files)):
          filename = files[i]
          parts = filename.split(path+'\\')
          nameonly = parts[len(parts)-1].split('.')[0]
          #nameonly = nameonly.split('_')[0]
          q=df.loc[df['Serial'] == nameonly]['h2'] #Changed number to INT because of new h2_full file 
          X_2[i]=log(q.values[0]) #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>CHJANGE THIS!!!
    else:
        for i in range(len(files)):
          filename = files[i]
          parts = filename.split(path+'\\')
    ...
    #      number=int(nameonly)
          ...
          q=df.loc[df['Serial'] == int(number)]['y'] #Changed number to INT because of new h2_full file 
          X_2[i]=q.values[0]
      
if not not additional_input2_file:
    df=pd.read_csv(additional_input2_file)  # Change da_dn here to get another property
    X_3=np.zeros((len(files),1))
    for i in range(len(files)):
      filename = files[i]
      parts = filename.split(path+'\\')
      nameonly = parts[len(parts)-1].split('.')[0]
...
if additional_input1_file:
    X_2=min_max_scaler.fit_transform(X_2)

min_max_scaler = preprocessing.MinMaxScaler()
if additional_input2_file:
...
if USE_ONLY_ADDITIONAL_INPUTS is False and additional_input1_file:
    df_master = pd.DataFrame(x_scaled)
    df_master['x2']=X_2
    df_master['x3']=X_3

elif USE_ONLY_ADDITIONAL_INPUTS is True and additional_input2_file:      
...
def train_test_splitter(df):
    a=pd.unique(df.Y)
    trainer,tester=train_test_split(a,test_size=0.1, shuffle=True)
