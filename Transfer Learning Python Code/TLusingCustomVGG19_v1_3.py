# -*- coding: utf-8 -*-
"""
Created on Mon Feb 14 09:50:48 2022
...
Use_Only_Images=False # If you only like to use iamges
Use_Only_Second_Input=True # if you just want to use the second property
...
property_file='G_avg_div_E.csv'

if Use_Both_Inputs:
    Extra_Input=True # If you like to add extra inputs along with iamge
else:
    Extra_Input=False
    
if Use_Only_Second_Input:
    Extra_Input=True # If you like to add extra inputs along with iamge
...
if Extra_Input:
    ...
    if additional_input_file=="h2_full.csv":
        for i in range(len(files)):
          filename = files[i]
          parts = filename.split(path+'\\')
          nameonly = parts[len(parts)-1].split('.')[0]
