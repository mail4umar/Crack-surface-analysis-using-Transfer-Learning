# -*- coding: utf-8 -*-
"""
Created on Thu Apr  7 23:23:46 2022
...
new in v2_4: combined PCA of both images and SDF features
...
desired_block_images='block3_pool'
desired_block_sdf_images='block3_pool'
pca_original=False # Either this OR combined PCA! not both!
correlation_CUTOFF=0.5

remove_correlated=True #if we want to remove correlated terms
PCA_components_images=5
PCA_components_sdf_images=8

please_combined_pca=True
PCA_combined=11
Blind_test=True
...
if please_combined_pca:
    features=np.hstack((pic_features, select_sdf_features))
    pca=PCA(n_components=PCA_combined)
    pca_vectors = pca.fit_transform(features)
    df_master=pd.DataFrame(pca_vectors)
    print('PCAs from IMAGES & SDF images are:',pca_vectors.shape[1])
    print("IMAGE & SDF image variance explaiend=",sum(pca.explained_variance_ratio_))
