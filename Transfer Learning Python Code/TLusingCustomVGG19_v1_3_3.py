# -*- coding: utf-8 -*-
"""
Created on Thu Mar 10 09:43:50 2022
...
additional_input_file="h2_full.csv" #h2_full please IF you want h2_full. There is a condition
property_file='a0.csv'
desired_block='block5_pool'
PCA_components=13


if Use_Only_Images or Use_Only_Second_Input:
    Use_Both_Inputs=False # If you like to use both images and second property like h2,temp etc
else:
    Use_Both_Inputs=True
...
path3="\\Python\\TransferLearning\\ford\\SDF_Umar"
...
if Extra_Input:
    df=pd.read_csv(additional_input_file)  # Change da_dn here to get another property
    X_2=np.zeros((len(files),1))
    if additional_input_file=="h2_full.csv" or additional_input_file=="h2_Donut.csv":
        for i in range(len(files)):
          filename = files[i]
          parts = filename.split(path+'\\')
          nameonly = parts[len(parts)-1].split('.')[0]
